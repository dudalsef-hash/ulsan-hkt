...
import { getLanguageDisplayName, getCurrencySymbol, getDefaultBudget, SUPPORTED_CURRENCIES } from "../lib/language-config";
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (settings.people) setPeople(settings.people);
      }
  ...
  useEffect(() => {
    try {
      localStorage.setItem("safeplate-settings", JSON.stringify({ currency, budget, people }));
    } catch { /* 저장 실패 무시 */ }
...
function SetupScreen({
  ...
  const budgetDefaults = useMemo(() => getDefaultBudget(currency), [currency]);
      ...
      <section className="form-card">
        ...
        <label className="budget-control">
          ...
          <div className="budget-input">
            <button type="button" className="currency-selector" onClick={() => setShowCurrencyPicker(!showCurrencyPicker)}>{currencySymbol} {currency} ▼</button>
...
function createSampleAnalysis(): MenuAnalysisResult {
  return {
    ...
    restaurant_context: "Safe Plate 샘플 메뉴 (일본 식당)",
    menus: [

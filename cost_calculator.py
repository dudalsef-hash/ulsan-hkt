...
class CostCalculator:
    ...
    def print_summary(self, mode: str = "equal") -> str:
        ...
        for item in result.items_summary:
            lines.append(
                f"  {item['name']:12s} | {item['unit_price']:>8,}원 × {item['quantity']}개 = {item['subtotal']:>10,}원"
            )
        lines.append("-" * 50)
        lines.append(f"  {'총액':12s} | {result.total_amount:>10,}원")
        lines.append("")
        lines.append(f"■ 참가 인원: {result.num_participants}명")
        lines.append(f"■ 분배 방식: {mode}")
        lines.append("")
        lines.append("■ 1인당 분배 금액")
        lines.append("-" * 50)
        for name, amount in result.per_person_detail.items():
            lines.append(f"  {name:12s} → {amount:>10,}원")
        lines.append("-" * 50)

        if result.remainder != 0 and mode == "equal":
            lines.append(f"  ※ 나머지 {result.remainder:,}원은 첫 번째 참가자에게 추가됨")
